...
export function parseCliArgs(argv: string[]): {
  ...
  if (argv[2] === "adopt") {
    ...
    for (let i = 3; i < argv.length; i++) {
      if ((argv[i] === "--template" || argv[i] === "-t") && argv[i + 1]) template = argv[++i];
    }
    return { ...defaults, runAdopt: true, adoptTemplate: template };
  }
  if (argv[2] === "history") {
  ...
  if (argv[2] === "sync") {
    ...
    return { ...defaults, runSync: true, syncAction: action, syncRemote: remote };
  }
  if (argv[2] === "doctor") {
  ...
  if (argv[2] === "attach") {
    return { ...defaults, runAttach: true };
  }
